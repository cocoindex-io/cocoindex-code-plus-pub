...
| **Audit** (what the log keeps) | `audit.{freeText,hmacKeySecret,logResultPaths}` | default (`freeText: redact`) | the query server's structured JSON audit stream is always on; these govern its field policy. `freeText` — `redact` (default) / `hmac` (keyed pseudonyms, requires `hmacKeySecret`) / `plain` (dev) — covers **all** caller free text: query text, grep patterns, path globs, paths, symbol targets. `logResultPaths` (default `false`) adds result-derived filenames/refs. Setting any of these flips auth onto the **file lane** (see **API tokens** above). Event shapes and the field matrix: [security.md](security.md#logging--audit) |
